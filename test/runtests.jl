...
@testset "Integration" begin
    @testset "Trapizoid" begin
        @test trapizoid(sq, 0., 1., 100) ≈ 1/3 atol=1e-4
        @test trapizoid(sq, 0., 1., 1000) ≈ 1/3 atol=1e-5
        @test trapizoid(sq, 0., 1., 10000) ≈ 1/3
    end
    @testset "Simpson" begin
        @test simpson(sq, 0., 1., 100) ≈ 0.3333333333333333 atol=1e-5
        @test simpson(sq, 0., 1., 1000) ≈ 0.3333333333333333
        @test simpson(sq, 0., 1., 10000) ≈ 0.3333333333333333
    end
end

@testset "Root Finding" begin
    @testset "Bisection" begin
        @test bisection(f, 0., 2.) ≈ 1.0 atol=1e-5
    ...
    @testset "Newton Method" begin
        ...
        @test abs(imag(NewtonMethod(f,df,x0*(1+im)))) ≈ 0.
        @test NewtonMethod(x->[x[1]^2 - 2, x[2]^2 - 4], x->[2x[1] 0; 0 2x[2]], [x0, x0]) ≈ [√2, 2]
    end 
    @testset "Brent" begin
        @test brent(f, 0., 2.) ≈ 1.0 atol=1e-5
        @test brent(sq, 0., 2.) ≈ 0.0 atol=1e-5
    end
